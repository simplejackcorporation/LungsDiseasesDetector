...
def find_right_lung_haar(image):
    right_lung = cv2.CascadeClassifier(os.path.dirname(__file__) + os.sep + "right_lung_haar.xml")
...
def find_left_lung_haar(image):
    left_lung = cv2.CascadeClassifier(os.path.dirname(__file__) + os.sep + "left_lung_haar.xml")
